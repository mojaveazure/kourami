...
function usage {
    ...
    echo "Note: Use this if you have bam file aligned to GRCh38 (primary assembly + decoy + ALT +  HLA from [bwa-kit] : 38DH)"
    ...
    echo
    echo " bamfile          : sorted and indexed bam to hs38DH (ex: NA12878.bam) [required]"
    echo
    echo "------------------ Optional Parameters -----------------"
    echo " -d [panel DB]    : Path to Kourami panel db. [Default: db directory under Kourami installation kourami/db]"
    echo
    echo " -t [CRAM ref]    : path to optional reference for CRAM input"
    echo
...
while getopts :d:r:t:l:h FLAG; do
    case $FLAG in
    d)
        kourami_db=$OPTARG
        ;;
...
if [ ! -x "$samtools_bin" ] || [ ! -x "$bwa_bin" ] || [ ! -x "$bamUtil" ] || [ ! -x "$parallel_bin" ]; then
...
if [ -z "${reference_fasta}" ]; then
    ...
    if [ -f ${reference_fasta} ]; then
        [ -f "${reference_fasta}.fai" ] || $samtools_bin faidx ${reference_fasta}
        reference="-T ${reference_fasta}"
    else
        echo "Cannot find reference fasta provided"
        exit 1
    fi
fi
...
if [ ! -e "$bam_path" ] || [ ! -e "$kourami_db" ] || [ ! -e "$merged_hla_panel" ] || [ ! -e "$grch38_HLA_NoAlt" ];then
    echo "Missing one of the following files/directories (38DH):"
    echo -e "  input Bam file             : $bam_path"
    echo -e "  Kourami Formatted IMGT DB  : $kourami_db"
    echo -e "  Kourami Panel sequences    : $merged_hla_panel"
    echo -e "  reference genome sequence  : $grch38_HLA_NoAlt"
    exit 1
fi

if [ ! -f "$grch38_HLA_NoAlt_index" ];then
    echo "hs38NoAltDH must be bwa-indexed first."
    echo "Please run bwa index $grch38_HLA_NoAlt_index"
    exit 1
fi
...
echo $(date)
$parallel_bin --verbose "$samtools_bin view ${reference} -b ${bam_path} {} > ${TMPDIR}/{}.bam" :::: ${contigs_list}
$samtools_bin merge "${TMPDIR}/merged.bam" $(find ${TMPDIR} -type f -name "*.bam")
$samtools_bin view -b -F 0x4 "${TMPDIR}/merged.bam" | \
    $samtools_bin sort --thread $num_processors -m $samtools_sort_memory_per_thread -O BAM - > "$sampleid.tmp.extract.bam"
rm -rf ${TMPDIR}
...
OUT=$?
if [ ! $OUT -eq 0 ];then
    echo '$bamUtil fastq extraction Failed! (38DH)'
    exit 1
else
    rm $sampleid.tmp.extract.bam*
fi
...
if [ ! $OUT -eq 0 ];then
    ...
    rm $sampleid\_tmp.extract_?.fq.gz $sampleid\_tmp.extract.unpaired.fq.gz
fi

$samtools_bin index $sampleid\_tmp.extract_on_grch38.bam

$samtools_bin view -b $sampleid\_tmp.extract_on_grch38.bam \
    chr6:29723340-29727296 chr6:29726601-29749049 chr6:29826979-29831122 \
    chr6:29887760-29891080 chr6:29942470-29945884 chr6:30005971-30009956 chr6:30259562-30266951 \
    chr6:30489406-30494205 chr6:31268749-31272136 chr6:31353866-31357245 chr6:31399784-31415316 \
    chr6:31494881-31511124 chr6:32439842-32445051 chr6:32517377-32530229 chr6:32552713-32560002 \
    chr6:32578770-32589836 chr6:32637406-32643652 chr6:32659464-32666689 chr6:32741386-32746887 \
...
if [ ! $OUT -eq 0 ];then
    ...
    rm $sampleid\_tmp.extract_on_grch38.bam*
fi
